...
async def help(ctx, *, member : discord.Member  = None):
    ...
    embed.add_field(name="`?userinfo <@user>`", value="Informations about you or another user..", inline=False)
    embed.add_field(name="`?ban <@user> <reason>`", value="Ban user on your server!", inline=False)
    embed.add_field(name="`?kick <@user> <reason>`", value="Kick user on your server!", inline=False)
    embed.add_field(name="`?mute <@user> <reason>`", value="Mute user with this command!", inline=False)     
    embed.add_field(name="`?unmute <@user> [reason]`", value="You can unmute user with this magical command!", inline=False)
    embed.add_field(name="`?warn <@user> <reason>`", value="Warn user on your server!", inline=False)
    embed.add_field(name="`?say <message>`", value="BOT can say your words, but you need write it!", inline=False)
    embed.add_field(name="`?clear <number>`", value="Clear messages on your server", inline=False)
    embed.add_field(name="`?ping`", value="Do you want know your ping?", inline=False)
...
@client.command(pass_context = True)
async def invite(): 
    embed = discord.Embed(description ="[Invite me here!](https://discordapp.com/api/oauth2/authorize?client_id=509080817000251402&permissions=8&scope=bot)")
...
@client.command(pass_context=True)
async def kick(ctx, userName: discord.User, *, message:str):
...
@client.command(pass_context = True)
async def mute(ctx, userName: discord.User, *, message:str):
     if ctx.message.author.server_permissions.administrator or ctx.message.author.id == '194151340090327041':
...
@client.command(pass_context = True)
async def unmute(ctx, userName: discord.User, *, message:str):
     if ctx.message.author.server_permissions.administrator or ctx.message.author.id == '194151340090327041':
        role = discord.utils.get(member.server.roles, name='Muted')
